 ...
 */
export async function isMcpServerRunning(args) {
  let client = null;
  try {
 ...
 */
export async function startMcpServerForTestingPurposes() {
 ...
 */
export function isMcpServerRunningSync() {
  return mcpServerProcess !== null && !mcpServerProcess.killed;
}
/**
 * Generate a valid JWT token for testing
 */
export function generateValidToken(expiresIn = 3600) {
 ...
 */
export function generateTokenWithoutExpiry() {
  const now = Math.floor(Date.now() / 1000);
  const payload = {
    sub: 'test-client',
    iat: now,
...
export async function createAuthenticatedMcpClient(args) {
  const timeout = 5000;
