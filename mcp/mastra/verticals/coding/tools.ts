...
export const listUserRepositories = createTool({
    ...
    description: 'Lists all public repositories for a given GitHub username. Returns repository information including name, description, stars, and language. Defaults to "ffMathy" if no username is provided.',
    inputSchema: z.object({
        username: z.string().optional().describe('The GitHub username to list repositories for (defaults to "ffMathy" if not provided)'),
    }),
...
export const listRepositoryIssues = createTool({
    ...
    description: 'Lists all issues for a specific GitHub repository. Can filter by state (open/closed/all). Defaults to "ffMathy" owner if not specified.',
    inputSchema: z.object({
...
export const searchRepositories = createTool({
    ...
    description: 'Searches for GitHub repositories by name or keywords. Returns matching repositories with their details. Defaults to filtering by "ffMathy" owner if not specified.',
    inputSchema: z.object({
...
export const assignCopilotToIssue = createTool({
    ...
    description: 'Provides instructions for assigning GitHub Copilot Coding Agent to a specific issue. This is an informational tool that does NOT automatically start a coding task. To enable automation, install the GitHub App and configure the MCP server with proper permissions. Defaults to "ffMathy" owner if not specified.',
    inputSchema: z.object({
        owner: z.string().optional().describe('The repository owner (defaults to "ffMathy" if not provided)'),
        repo: z.string().describe('The repository name (e.g., "hey-jarvis")'),
