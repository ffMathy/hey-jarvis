...
**Sample mode shows the frame rate in the corner**, counted on the UI thread where the frames happen and read twice a second by a leaf component. It reports what the device achieves, so thirty means it is keeping up with `MINIMUM_FRAME_SECONDS` and less is what it managed. Use it before and after any change made for speed.
