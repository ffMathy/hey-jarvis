...
`build:apk` depends on `initialize`, which puts React Native Skia's prebuilt libraries where its CMake build looks for them. Postinstalls never run in this repository (`ignoreScripts` in `bunfig.toml`), and without that step the build dies three minutes in with "Skia prebuilt binaries not found" — which is how the watch app's first CI build failed. It runs from `watch/` deliberately: Bun's isolated linker keys its store by a package's whole dependency closure, so this app and the phone app resolve different copies of Skia, and each copy needs its own libraries put in place.
